...
export interface JobProgressState {
  ...
  trials: TrialPoint[]
  socketOpen: boolean
  connectionError: 'unauthorized' | 'forbidden' | 'network' | null
}
...
type Action =
  ...
  | { type: 'connection-error'; error: JobProgressState['connectionError'] }
  | { type: 'reset' }
...
function reducer(state: JobProgressState, action: Action): JobProgressState {
  if (action.type === 'socket') {
    return { ...state, socketOpen: action.open, connectionError: action.open ? null : state.connectionError }
  }
...
export function useJobProgress(jobId: string | null, options: UseJobProgressOptions = {}): JobProgressState {
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      ...
      const socket = new WebSocket(jobSocketUrl(jobId), ['bearer', token])
      ws = socket
      let opened = false
      ...
      socket.onclose = (event) => {
        ...
        attempts += 1
        dispatch({ type: 'connection-error', error: 'network' })
